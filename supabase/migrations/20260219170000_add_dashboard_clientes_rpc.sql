-- Função para retornar dados reais do dashboard de clientes (evita limite 1000 do PostgREST)
...
DECLARE
  ...
  v_result json;
BEGIN
  SELECT COUNT(*) INTO v_total FROM clients;
  SELECT COUNT(*) INTO v_novos FROM clients WHERE created_at >= p_from AND created_at <= p_to;
