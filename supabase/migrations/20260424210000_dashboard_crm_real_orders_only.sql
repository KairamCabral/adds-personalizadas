...
BEGIN
  ...
  kpis AS (
    SELECT
      ...
      -- ATIVOS: pedidos CRM, não arquivados, não status terminal, não cancelados
  ...
  funil AS (
    ...
    SELECT 'Em andamento',
      GREATEST(0,
        (SELECT COUNT(*)::int FROM funil_entrada)
        - (SELECT COUNT(*)::int FROM funil_concluidos)
        - (SELECT COUNT(*)::int FROM funil_cancelados)
      ),
      4
  ),
...
'Dashboard CRM: regras 24/04/2026 (pedidos reais) — apenas pedidos created_at >= 2026-03-01 OR com order_history; 3 cards separados (cancelados/arquivados/excluidos); FATURADO excluído de tempo por etapa.';
